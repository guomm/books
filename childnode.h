...
private:
	...
	unsigned char *bufSend = (unsigned char *)malloc(1024*1024);//SA+suffixGIndex+Type+isG+s;
...
public:
	...
	void computeSA(unsigned char *s, int64 *SA, int64 *GRank, int64 *suffixGIndex, int64 n, int32 cs, int64 K, int64 level) {
		...
		induceSAl(t,SA,s,bkt,n,K,cs,level, GRank, suffixGIndex, nodeId, status);

		induceSAs(t, SA, s, bkt, n, K, cs, GRank, suffixGIndex, nodeId, status);
		...
		int64 n1 = 0, sendPos = 0, offset = 0, desOffset = 0;
		for (i = 0; i<n; i++)
			if (isLMS(SA[i])) {
				SA[n1++] = SA[i];
		...
		//desOffset = offset + n1;
		int64 j = 0;
		for (i = n - n1; i < n; i++)suffixGIndex[j++] = SA[i];
...
//		int64 *SA1 = SA, *s1 = SA + n - n1;
		...
		j = 0;
		for (i = n - n1; i<n; i++) suffixGIndex[s1[j++]] = SA[i]; // lms字符的suffixGIndex已放好

		j = 0;
		for (i = n-n1; i<n; i++) SA[i] = SA1[j++]; // 
		SA1 = SA + n - n1;

		/*cout << "level is " << level << " ppgggggppppppp:" << endl;
		for (i = 0; i < n1; i++)cout<<SA1[i]<<","<<s1[i]<<endl;*/

		for (i = 0; i < n1; i++)SA[SA1[i]] = s1[i];
		cout << "level is " << level << " pppppppppppppppp:" << endl;
		for (i = 0; i < n1; i++) {
			cout << SA[i] << endl;
		}
		...
		cout << "level is " << level << "  n1 is "<<n1 <<" n is "<<n<<endl;
		for (i = 0; i < n; i++) {
			cout << SA[i] << "," << suffixGIndex[i] << endl;
		}
	...
	void induceSAl(unsigned char *t, int64 *SA, unsigned char *s, int64 *bkt,
		int64 n, int64 K, int64 cs, int64 level, int64 *GRank , int64 *suffixGIndex,int32 nodeId, MPI_Status &status) {
		int64 i, j, sendPos = 0;
	...
	void induceSAs(unsigned char *t, int64 *SA, unsigned char *s, int64 *bkt,
		int64 n, int64 K, int64 cs, int64 *GRank, int64 *suffixGIndex, int32 nodeId, MPI_Status &status) {
	...
	void compute(string fileName) {
		...
		for (int64 i = 0; i < n; i++) {
			GRank[suffixGIndex[i]-1] = i;
		}
		for (int64 i = 0; i < n; i++) {
			if (GRank[i] != SA[i]) cout << "error" << endl;;
		}
